...
def _projection_email_html(projection: Optional[dict]) -> str:
    """Monte Carlo fan summary for the public email. Index base-100 only."""
    if not projection or not projection.get("available"):
        note = (projection or {}).get("note") or "Projection unavailable this week."
        return (
            f'<p style="margin:12px 0 0;font-size:13px;color:#5A646E">'
            f'<b>Monte Carlo (52w)</b> — {note}</p>'
        )
    s = projection.get("summary") or {}
    corr = "correlated" if projection.get("correlated") else "independent (degenerate cov)"
    p10 = s.get("p10", "—")
    p50 = s.get("p50", s.get("median", "—"))
    p90 = s.get("p90", "—")
    vol = projection.get("annualized_vol_pct")
    vol_line = f"{vol}% ann. vol (trailing)" if vol is not None else "vol n/a"
    note = projection.get("note") or ""
    return f"""
    <div style="margin-top:14px;padding-top:12px;border-top:1px solid #E2DAD3">
      <div style="font-size:14px;font-weight:700;color:#171A1D;margin-bottom:6px">
        Monte Carlo path — 52 weeks (index base 100)</div>
      <div style="font-size:13px;color:#5A646E;margin-bottom:8px">
        {corr} shocks · {projection.get("holdings_simulated", "—")} holdings ·
        {projection.get("trailing_weeks_sampled", "—")} trailing weeks · {vol_line}</div>
      <table style="width:100%;border-collapse:collapse;font-size:13px">
        <tr style="border-bottom:1px solid #E2DAD3">
          <td style="padding:6px 10px">p10</td>
          <td style="padding:6px 10px;text-align:right;font-weight:700">{p10}</td>
          <td style="padding:6px 10px;color:#5A646E">lower decile (not a worst case)</td>
        </tr>
        <tr style="border-bottom:1px solid #E2DAD3">
          <td style="padding:6px 10px">p50</td>
          <td style="padding:6px 10px;text-align:right;font-weight:700">{p50}</td>
          <td style="padding:6px 10px;color:#5A646E">median path</td>
        </tr>
        <tr>
          <td style="padding:6px 10px">p90</td>
          <td style="padding:6px 10px;text-align:right;font-weight:700">{p90}</td>
          <td style="padding:6px 10px;color:#5A646E">upper decile</td>
        </tr>
      </table>
      <p style="margin:8px 0 0;font-size:12px;color:#5A646E">{note}</p>
    </div>
"""
...
def write_email_fragment(
    ...
    """The PUBLIC email section. Tickers, weights, scores, MC band. No dollars."""
    ...
    mc_html = _projection_email_html(projection)
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    write_email_fragment(b, call, week_ending, dry_run=args.dry_run, projection=projection)
