# =============================================================================
#  MalikAI-786 SPX v5.0  -  Perplexity AI-Peer Adapter
# -----------------------------------------------------------------------------
...
#  ... AND a valid PERPLEXITY_API_KEY in the environment.
#
#  Perplexity is intentionally included as the third peer because its
#  retrieval-augmented architecture grounds answers in live web sources -
#  a useful counterweight to the two pure LLM peers (Codex, Gemini).
#
...
MODEL_VERSION_PINNED = "sonar-large-online-2026-01"
PROMPT_TEMPLATE_PATH = "sources/prompt_template.md"
LIVE_SENTINEL_ENV = "MALIKAI_PERPLEXITY_LIVE"
...
def _placeholder_response() -> dict[str, Any]:
    ...
    return {
        ...
        "source": SOURCE_NAME,
        "note": "placeholder for v5.0 - wire to Perplexity AI via API",
    }
...
# Canonical text lives in prompt_template.md. Inlined here so that the
# raw request body logged into responses/ is self-describing.
PROMPT_TEMPLATE = """\
...
def get_perplexity_bias(date_iso: str, market_context: dict[str, Any]) -> dict[str, Any]:
    """
    Return Perplexity AI's bias read for the given trading date.
    ...
    dict
        Live mode:  payload conforming to the schema in prompt_template.md,
                    with `citations` populated from Perplexity's own
                    web-source attributions.
        Placeholder mode (default in v5.0):
            {"status": "NOT_WIRED_YET", "source": "perplexity", "note": "..."}

    Notes
    -----
    Safe to call without an API key. Only attempts a live API call if BOTH:
      * env var MALIKAI_PERPLEXITY_LIVE == "1"
      * env var PERPLEXITY_API_KEY is non-empty
    """
    ...
    # Perplexity exposes an OpenAI-compatible REST endpoint over HTTPS.
    #
    #   import requests
    ...
    #   # Merge in Perplexity's own citations if missing from model output
    #   payload.setdefault("citations", data.get("citations", []))
    ...
    return {
        ...
        "source": SOURCE_NAME,
        "note": "MALIKAI_PERPLEXITY_LIVE=1 but adapter body still placeholder; "
